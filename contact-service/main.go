...
func main() {
	...
	addrRepository := fmt.Sprintf("postgres://%s:%s@postgres/%s?sslmode=disable", cfg.PostgresUser, cfg.PostgresPassword, cfg.PostgresDB)
	repo, err := database.CNDatabase(addrRepository)
	if err != nil {
		log.Fatal(err)
	}
